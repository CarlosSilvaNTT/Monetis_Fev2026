...
public class AccountsPage {
    ...
    private static final By NAV_PAYMENTS = By.xpath(
            "//div[@class='navigation']//div[@class='options']/div[span[normalize-space()='Payments']]"
    );
    ...
    public void clickPaymentsNav() {
        waitLoaded();
        WebElement payments = wait.until(ExpectedConditions.elementToBeClickable(NAV_PAYMENTS));
        ((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView({block:'center'});", payments);
        try { payments.click(); }
        catch (ElementClickInterceptedException e) {
            wait.until(ExpectedConditions.invisibilityOfElementLocated(LOADING_OVERLAY));
            ((JavascriptExecutor) driver).executeScript("arguments[0].click();", payments);
        }
